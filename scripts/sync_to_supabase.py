...
Usage:
    ...
    python scripts/sync_to_supabase.py --predictions-only --date 20260321  # 予測のみ
"""
...
TABLE_COLUMNS = {
    "horses": [
        "horse_id", "name", "sex", "birth_year",
        "father", "mother", "trainer", "owner",
    ],
    "races": [
        "race_id", "date", "venue", "race_number", "race_name",
        "grade", "distance", "surface", "direction",
        "weather", "track_condition", "horse_count",
    ],
    "entries": [
        "race_id", "horse_id", "bracket_number", "horse_number",
        "horse_name", "sex_age", "weight_carried", "jockey", "trainer",
        "odds", "popularity", "horse_weight", "horse_weight_change",
    ],
    "race_results": [
        "race_id", "horse_id", "finish_position", "bracket_number",
        "horse_number", "horse_name", "sex_age", "weight_carried",
        "jockey", "finish_time", "margin", "speed_figure",
        "passing_order", "last_3f", "horse_weight", "horse_weight_change",
...
def sync_predictions_only(client, conn: sqlite3.Connection, date_str: str):
    ...
    for table in ["predictions", "prediction_results"]:
        ...
        rows = conn.execute(
            f"SELECT {cols} FROM {table} WHERE race_id IN ({placeholders})",
            race_id_list
        ).fetchall()
